...
function treeHalfWidth(t){
  ...
  const base = 0.035 + 0.30 * Math.pow(t, 0.92);

  // 2 tầng phình
  const b1 = 0.11 * Math.exp(-Math.pow((t - 0.34) / 0.10, 2));
  ...
  // bo nhẹ
  const smooth = 0.010 * Math.sin(t * Math.PI * 2.6);
...
let outlineX=null, outlineY=null, outlineN=0;
...
function drawCenteredText(line1, line2, alpha, yPx){
  ...
  if(line2){
    ...
    ctx.font = `600 ${Math.max(14, baseSize*0.42)}px system-ui, -apple-system, Segoe UI, Roboto, Arial`;
...
function loop(now){
  ...
  const lineW = Math.max(2.6, baseScale*0.010);

  drawGlowPathPartial(outlineX, outlineY, outlineN, drawP, glowColor, lineW);

  const starProg = clamp(drawP*1.10, 0, 1);
  drawGlowPathPartial(starX, starY, starN, starProg, glowColor, Math.max(2.2, baseScale*0.008));
  ...
  if(drawP > 0.83){
    ...
    ctx.arc(cx, y, baseScale*0.10, 0, Math.PI, false);
    ctx.stroke();
    ctx.restore();
  }

  // TEXT: đặt theo base line (giống mẫu) + clamp để không tụt
  const yText = Math.min(H - 110, baseLineY + 0.30*baseScale);

  if(merryA > 0){
    drawCenteredText('Merry Christmas!!', '', merryA, yText);
  }
  if(wishA > 0){
    drawCenteredText('Wish you...', 'Happiness • Health • Success', wishA, yText);
  }
